...
function preguntaAlPublico() {
    ...
    // Obtener los elementos hijos del div con el id = oculto
    ...
    for (let i = 0; i < res.length; i++) {
        if (res[i].tagName === "BUTTON") {
            if (!res[i].disabled) { // Verificar si el botón no está deshabilitado
                cont++;
    ...
    // Aplicar el efecto de borroso al botón "comodinPublico"
    let botonPublico = document.getElementById("comodinPublico");
   // botonPublico.style.filter = "blur(3px)";
